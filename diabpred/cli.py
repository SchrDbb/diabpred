...
# Show help
python -m diabpred.cli --help
python -m diabpred.cli predict --help
"""
...
# Allow running as `python -m diabpred.cli`
...
def cmd_predict(args: argparse.Namespace) -> None:
    ...
    from diabpred.models import load_model, train_model
    ...
    # Load or train the requested model
    ...
    # We need a scaler — train from scratch to get one
    df = load_dataset()
    ...
    if args.ensemble:
        ...
        all_models = train_all_models(X_train if 'X_train' in dir() else preprocess(df)[0],
                                      y_train if 'y_train' in dir() else preprocess(df)[2],
                                      verbose=False)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_pred = sub.add_parser(
        "predict", help="Predict diabetes risk for a single patient",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p_pred.add_argument("--pregnancies", type=float, required=True, help="Number of pregnancies")
    p_pred.add_argument("--glucose",     type=float, required=True, help="Plasma glucose (mg/dL)")
    p_pred.add_argument("--bp",          type=float, required=True, help="Diastolic blood pressure (mmHg)")
    p_pred.add_argument("--skin",        type=float, required=True, help="Triceps skinfold thickness (mm)")
    p_pred.add_argument("--insulin",     type=float, required=True, help="2-hour serum insulin (μU/mL)")
    p_pred.add_argument("--bmi",         type=float, required=True, help="BMI (kg/m²)")
    p_pred.add_argument("--dpf",         type=float, required=True, help="Diabetes pedigree function")
    p_pred.add_argument("--age",         type=float, required=True, help="Age (years)")
    p_pred.add_argument("--model",       default="Random Forest", help="Model to use for prediction")
    p_pred.add_argument("--model-dir",   default="models/",       help="Directory containing saved .pkl models")
    p_pred.add_argument("--threshold",   type=float, default=0.5,  help="Decision threshold (0–1)")
    p_pred.add_argument("--ensemble",    action="store_true",       help="Show probabilities from all models")
    p_pred.add_argument("--json",        action="store_true",       help="Also print result as JSON")
    p_pred.set_defaults(func=cmd_predict)

    # ── benchmark ─────────────────────────────────────────────────────────────
    p_bench = sub.add_parser(
        "benchmark", help="Run the full benchmark experiment",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
